...
class EditAudio extends Component {
    ...
    handleAudioPartialRender() {
        ...
        if (wavesurfer) {
            const newBuffer = wavesurfer.backend.ac.createBuffer( // 创建新buffer
                originalBuffer.numberOfChannels,
                Math.ceil(defaultEnd - defaultStart) * originalBuffer.sampleRate, // 获取想要获取的波形范围
                originalBuffer.sampleRate
            );

            const newChannelData = new Float32Array((defaultEnd - defaultStart) * originalBuffer.sampleRate); // 新建新的通道数据
            ...
            newBuffer.copyToChannel(newChannelData, 0, 0); // 将新建新的通道波形数据付给新buffer
